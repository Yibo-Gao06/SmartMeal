...
public enum ResultCode {
     ...
     * 连续登录失败导致的临时锁定。
     *
     * <p>为什么不复用 {@link #PASSWORD_ERROR}：这两个给用户的**行动指引完全不同** ——
     * 前者是「再试一次」，后者是「等 15 分钟」。塞进同一个码里，
     * 前端只能靠解析文案来决定要不要禁用按钮，太脆。
     *
     * <p>也不用通用的 {@link #TOO_MANY_REQUESTS}(429)：那个是限流语义，
     * 客户端拿到后的标准动作是退避重试，而这里退避也没用，得等锁定自然过期。
     */
    AUTH_LOCKED(10005, "登录失败次数过多，账号已临时锁定"),
